...
struct HorizontalProgressBarRepresentable: UIViewRepresentable {
    var progress: CGFloat
    var bgColor: UIColor = UIColor.white
    var barColor: UIColor = UIColor(red: 52/255, green: 181/255, blue: 240/255, alpha: 1)
    var frameColor: UIColor = UIColor(red: 161/255, green: 161/255, blue: 161/255, alpha: 1)
    var frameBold: CGFloat = 0.1
    var pgHeight: CGFloat = 20
    var pgWidth: CGFloat = 200

    func makeUIView(context: Context) -> HorizontalProgressBar {
        let progressBar = HorizontalProgressBar()
        progressBar.bgColor = bgColor
        progressBar.barColor = barColor
        progressBar.frameColor = frameColor
        progressBar.frameBold = frameBold
        progressBar.pgHeight = pgHeight
        progressBar.pgWidth = pgWidth
        progressBar.animateProgress(duration: 1.0, progressValue: progress) // You can adjust this as needed
        return progressBar
    }

    func updateUIView(_ uiView: HorizontalProgressBar, context: Context) {
        uiView.animateProgress(duration: 1.0, progressValue: progress)
    }
}
